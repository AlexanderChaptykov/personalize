...
def get_worst_ds2(
    ...
    spars_df = (
        spars_df.groupby(user_col, as_index=False)
...
def plot_sparse(ser, bins=22, sort=True, o_range=None, MIN_ITEMS=20, y=None):
    ...
    plt.axvspan(
        (ser.value_counts() > MIN_ITEMS).sum(),
        nuniq,
        alpha=0.3,
        color="red",
...
def reduce_sparsity(
    ...
    good_users = (
        df[user_col]
        .value_counts()[df[user_col].value_counts() > min_shows_per_user]
        .index
    )
    ...
    good_shows = (
        df[item_col]
        .value_counts()[df[item_col].value_counts() > min_user_per_show]
        .index
    )
